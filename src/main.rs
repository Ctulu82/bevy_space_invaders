use bevy::prelude::*; // 게임 루프 및 주요 구조체들을 사용하기 위해 Bevy의 기본 prelude를 가져옵니다.

// 이 프로젝트에서 사용할 모듈들을 선언합니다.
pub mod core; // 게임 로직을 담당하는 모듈을 담은 입니다.
...
fn main() {
    App::new() // 새로운 Bevy 애플리케이션을 생성합니다.
        .add_plugins((
            ...
            core::game::GamePlugin, // 이 게임의 로직을 처리하는 GamePlugin을 추가합니다.
        ))
        .run(); // 애플리케이션을 실행합니다.
}
